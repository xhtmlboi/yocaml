...
val with_cache : ?on:Eff.filesystem -> Path.t -> (Cache.t -> Cache.t Eff.t) -> unit Eff.t
(** [with_cache ?on path f] restores the cache from the given [path], executes
    ...
    the same [path].This helps avoid repeating [restore_cache] and [store_cache] calls manually
