  ...
  test_case "String.equal" `Quick (fun () ->
      ...
      let () =
        check
          (Error
             (V.With_message
                { given = "world"; message = "should be equal to hello" }))
          (V.String.equal "hello" "world")
      in
      ())
  ...
  test_case "String.not_equal" `Quick (fun () ->
      ...
      let () =
        check
          (Error
             (V.With_message
                { given = "hello"; message = "should not be equal to hello" }))
          (V.String.not_equal "hello" "hello")
      in
      ())
...
let test_string_not_empty =
  ...
  test_case "String.not_empty" `Quick (fun () ->
      ...
      let () =
        check
          (Error
             (V.With_message { given = ""; message = "should not be empty" }))
          (V.String.not_empty "")
      in
      ())
...
let test_string_not_blank =
  ...
  test_case "String.not_blank" `Quick (fun () ->
      ...
      let () =
        check
          (Error
             (V.With_message { given = "   "; message = "should not be blank" }))
          (V.String.not_blank "   ")
      in
      let () =
        check
  ...
  test_case "String.has_prefix" `Quick (fun () ->
      ...
      let () =
        check (Ok "hello world")
          (V.String.has_prefix ~prefix:"hello" "hello world")
      in
      let () =
        check (Ok "hello") (V.String.has_prefix ~prefix:"hello" "hello")
      in
      let () =
        check
          (Error
             (V.With_message
                { given = "world"; message = "should have prefix \"hello\"" }))
          (V.String.has_prefix ~prefix:"hello" "world")
      in
      ())
...
let test_string_has_suffix =
  ...
  test_case "String.has_suffix" `Quick (fun () ->
      ...
      let () =
        check (Ok "hello world")
          (V.String.has_suffix ~suffix:"world" "hello world")
      in
      let () =
        check (Ok "world") (V.String.has_suffix ~suffix:"world" "world")
      in
      let () =
        check
          (Error
             (V.With_message
                { given = "hello"; message = "should have suffix \"world\"" }))
          (V.String.has_suffix ~suffix:"world" "hello")
      in
      ())
        ...
        check
          (Error
             (V.With_message
                {
                  given = "hi"
                ; message = "should have length 5, but has length 2"
                }))
          (V.String.has_length 5 "hi")
      in
      ())
             ...
             (V.With_message
                {
                  given = "hi"
                ; message =
                    "should have length greater than 3, but has length 2"
                }))
          (V.String.length_gt 3 "hi")
      in
      let () =
        check
          (Error
             (V.With_message
                {
                  given = "abc"
                ; message =
                    "should have length greater than 3, but has length 3"
                }))
          (V.String.length_gt 3 "abc")
      in
      ())
             ...
             (V.With_message
                {
                  given = "hi"
                ; message =
                    "should have length greater than or equal to 3, but has \
                     length 2"
                }))
          (V.String.length_ge 3 "hi")
      in
      ())
        ...
        check
          (Error
             (V.With_message
                {
                  given = "hello"
                ; message = "should have length less than 5, but has length 5"
                }))
          (V.String.length_lt 5 "hello")
      in
      let () =
        check
          (Error
             ...
             (V.With_message
                {
                  given = "world"
                ; message =
                    "should have length less than or equal to 4, but has \
                     length 5"
                }))
          (V.String.length_le 4 "world")
      in
      ())
...
let test_string_contains_only =
  ...
  test_case "String.contains_only" `Quick (fun () ->
      let check = check (Testable.validated_value string) "should be equal" in
      let () =
        check (Ok "abc") (V.String.contains_only ~chars:[ 'a'; 'b'; 'c' ] "abc")
      in
      let () =
        check (Ok "a") (V.String.contains_only ~chars:[ 'a'; 'b'; 'c' ] "a")
      in
      let () =
        check (Ok "") (V.String.contains_only ~chars:[ 'a'; 'b'; 'c' ] "")
      in
      let () =
        check
          (Error
             (V.With_message
                {
                  given = "abcd"
                ; message = "should contain only characters from a, b, c"
                }))
          (V.String.contains_only ~chars:[ 'a'; 'b'; 'c' ] "abcd")
      in
      ())
...
let test_string_exclude_chars =
  ...
  test_case "String.exclude_chars" `Quick (fun () ->
      let check = check (Testable.validated_value string) "should be equal" in
      let () =
        check (Ok "abc") (V.String.exclude_chars ~chars:[ 'x'; 'y'; 'z' ] "abc")
      in
      let () =
        check (Ok "") (V.String.exclude_chars ~chars:[ 'x'; 'y'; 'z' ] "")
      in
      let () =
        check
          (Error
             (V.With_message
                {
                  given = "abcx"
                ; message = "should not contain characters x, y, z"
                }))
          (V.String.exclude_chars ~chars:[ 'x'; 'y'; 'z' ] "abcx")
      in
      ())
        ...
        check
          (Error
             (V.With_message
                {
                  given = "invalid"
                ; message = "not included in [hello; world; test]"
                }))
          (V.String.one_of valid_strings "invalid")
      in
      let () =
        check (Ok "hello")
          (V.String.one_of ~case_sensitive:false valid_strings "HELLO")
      in
      let () =
        check
          (Error
          ...
          (V.String.one_of ~case_sensitive:true valid_strings "HELLO")
      in
      ())
  ...
  test_case "String.where" `Quick (fun () ->
      ...
      let () =
        check
          (Error
             (V.With_message
                { given = "hello"; message = "unsatisfied predicate" }))
          (V.String.where is_uppercase "hello")
      in
      ...
      let () =
        check
          (Error
             (V.With_message
                { given = "hello"; message = "string 'hello' is not uppercase" }))
          (V.String.where ~message:custom_message is_uppercase "hello")
      in
      ())
  ...
  test_case "negate" `Quick (fun () ->
      ...
      let is_positive x =
        if x > 0 then Ok x
          ...
          Error
            (V.With_message
               { given = string_of_int x; message = "should be positive" })
      in
        ...
        check
          (Error
             (V.With_message
                {
                  given = "<value>"
                ; message = "should not satisfy the validator"
                }))
          (is_not_positive 5)
      in
      let () =
